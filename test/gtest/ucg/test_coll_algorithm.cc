...
TEST(ucg_plan_test, test_NAP) {
    ucs_status_t ret;
    ucg_plan_test *obj = NULL;
    ucg_builtin_plan_t *plan = NULL;
    ucg_builtin_config_t *config = NULL;
    ucg_plan_test_data_t data[] = {
        {2, 2, 0}, {2, 2, 1}, {2, 2, 2}, {2, 2, 3},
        {1, 3, 0}, {1, 3, 1}, {1, 3, 2},
    ...
    for (unsigned i = 0; i < sizeof(data) / sizeof(data[0]); i++) {
        obj = new ucg_plan_test(data[i].node_cnt, data[i].ppn, data[i].myrank);
        config = (ucg_builtin_config_t *)obj->m_planc->plan_config;
        config->trees.intra_tree_type = 0;
        config->NAP.init_allreduce_method = 0;
        config->NAP.final_allreduce_method = 0;
        ret = ucg_builtin_NAP_create(obj->m_builtin_ctx, UCG_PLAN_NAP, config,
                                    obj->m_group_params, &obj->m_coll_type, &plan);
        delete obj;
    ...
    for (unsigned i = 0; i < sizeof(data) / sizeof(data[0]); i++) {
        obj = new ucg_plan_test(data[i].node_cnt, data[i].ppn, data[i].myrank);
        config = (ucg_builtin_config_t *)obj->m_planc->plan_config;
        config->trees.intra_tree_type = 1;
        config->NAP.init_allreduce_method = 1;
        config->NAP.final_allreduce_method = 1;
        ret = ucg_builtin_NAP_create(obj->m_builtin_ctx, UCG_PLAN_NAP, config,
                                    obj->m_group_params, &obj->m_coll_type, &plan);
        delete obj;
